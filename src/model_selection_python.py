...
def forwardSelection(X, y, model_type ="linear",elimination_criteria = "aic", varchar_process = "drop",verbose=False):
...
def backwardSelection(X, y, model_type ="linear",elimination_criteria = "aic", varchar_process = "drop",verbose=False):

	X = __varcharProcessing__(X,varchar_process = varchar_process)
	return __backwardSelectionRaw__(X, y, model_type = model_type,elimination_criteria = elimination_criteria,verbose=False )


def bothSelection(X, y, model_type ="linear",elimination_criteria = "aic", varchar_process = "drop",verbose=False):

	X = __varcharProcessing__(X,varchar_process = varchar_process)
	return __bothSelectionRaw__(X, y, model_type = model_type,elimination_criteria = elimination_criteria,verbose=False)
...
def __forwardSelectionRaw__(X, y, model_type ="linear",elimination_criteria = "aic",verbose=False):
	...
	for i in range(X.shape[1]):
		...
		for j in other_cols:
			...
			aicvals = aicvals.append(pd.DataFrame([[j, AIC(X,model)]],columns = ["Cols","aic"]),ignore_index=True)
...
def __bothSelectionRaw__(X, y, model_type ="linear",elimination_criteria = "aic",verbose=False):
	...
	for i in range(X.shape[1]):
		...
		for j in selected_cols:
			...
			temp_cols.remove(j)
			model = regressor(y, X[temp_cols])
			aicvals = aicvals.append(pd.DataFrame([[j, AIC(X,model),'delete']],columns = ["Cols","aic",'way']),ignore_index=True)

		for j in removed_cols:
			model = regressor(y, X[selected_cols+[j]])
			aicvals = aicvals.append(pd.DataFrame([[j, AIC(X,model),'add']],columns = ["Cols","aic",'way']),ignore_index=True)
