...
def clean_df(X):
    ...
    columns = ['DateOfDeparture', 'DateBooked', 
    'state_dep', 'state_arr', 
    'week', 'month', 'weekday', 
    'holidays_dep', 'holidays_arr', 
    'Departure', 'Arrival']
...
def _merge_external_data(X):
        ...
        print("after first merge" , X_merged.shape)

        # web_searches_date_booked_dep = ext_data[['DateOfDeparture', 'AirPort', 'Arrival', 'search_intensity']]
        # web_searches_date_booked_dep = web_searches_date_booked_dep.rename(columns={
        #     'DateOfDeparture': 'DateBooked',
        #     'AirPort': 'Departure', 
        #     'search_intensity': 'search_intensity_date_booked_dep'})
        # X_merged = pd.merge(
        #     X_merged, web_searches_date_booked_dep, how='left', on=['DateBooked', 'Departure', 'Arrival'], sort=False)
        
        # web_searches_date_booked_arr = ext_data[['DateOfDeparture', 'AirPort', 'Arrival', 'search_intensity']]
        # web_searches_date_booked_arr = web_searches_date_booked_arr.rename(columns={
        #     'DateOfDeparture': 'DateBooked',
        #     'Arrival': 'Departure',
        #     'AirPort': 'Arrival', 
        #     'search_intensity': 'search_intensity_date_booked_arr'})
        # X_merged = pd.merge(
        #     X_merged, web_searches_date_booked_arr, how='left', on=['DateBooked', 'Departure', 'Arrival'], sort=False)
        ...
        print("After second merge: ", X_merged.shape)
        ...
        'Mean Wind SpeedKm/h', 'CloudCover', 'WindDirDegrees', 'LoadFactorDomestic',
        'PassengersDomestic', 'latitude_deg', 'longitude_deg', 'state', 'pop2010', 
        'UnemploymentRate', 'holidays', 'GDP_per_cap', 'closest_holidays', 'search_intensity_dep', 'search_intensity_arr']]
        ...
        airport_info_arr = airport_info_arr.rename(
            ...
            'search_intensity_dep': 'search_intensity_datedep_arr',
            'search_intensity_arr': 'search_intensity_datedep_arr'})
