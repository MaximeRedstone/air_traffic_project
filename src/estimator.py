...
def clean_df(X):
    ...
    columns = ['DateOfDeparture', 'DateBooked', 'state_dep', 'state_arr', 'week', 'month', 'weekday',
                'Departure', 'Arrival', 'holidays_dep', 'holidays_arr']
...
def _merge_external_data(X):
        ...
        date_booked_search_intensity = ext_data[['DateOfDeparture', 'AirPort', 'Arrival', 'search_intensity']]
        date_booked_search_intensity = date_booked_search_intensity.rename(columns={
            'DateOfDeparture': 'DateBooked', 'AirPort': 'Departure', 'search_intensity': 'search_intensity_booked_dep'})
        X_merged = pd.merge(X_merged, date_booked_search_intensity, how='left', on=['DateBooked', 'Departure', 'Arrival'], sort=False)

        date_booked_search_intensity = ext_data[['DateOfDeparture', 'AirPort', 'Arrival', 'search_intensity']]
        date_booked_search_intensity = date_booked_search_intensity.rename(columns={
            'DateOfDeparture': 'DateBooked', 'Arrival': 'Departure', 'AirPort': 'Arrival', 'search_intensity': 'search_intensity_booked_arr'})
        X_merged = pd.merge(X_merged, date_booked_search_intensity, how='left', on=['DateBooked', 'Departure', 'Arrival'], sort=False)
        ...
        'Total', 'Flights', 'Booths', 'Mean per flight', 'search_intensity']]
        ...
        airport_info_dep = airport_info_dep.rename(
            ...
            'search_intensity': 'search_intensity_dep'})
        ...
        'Mean Wind SpeedKm/h', 'CloudCover', 'WindDirDegrees', 'LoadFactorDomestic',
        'PassengersDomestic', 'latitude_deg', 'longitude_deg', 'state', 'pop2010', 
        'UnemploymentRate', 'holidays', 'GDP_per_cap', 'closest_holidays', 'search_intensity']]
        ...
        airport_info_arr = airport_info_arr.rename(
            ...
            'search_intensity': 'search_intensity_arr'})
